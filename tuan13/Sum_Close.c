...
// Hàm duyệt cây nhị phân và tính tổng giá trị của các node chỉ có một nút con
void duyet(TreeNode* node, int* Sum) {
    // Nếu node là NULL, dừng hàm
    if (node == NULL)
        return;
    // Nếu node có một con trái hoặc một con phải, cộng giá trị của node vào biến Sum
    if ((node->left == NULL) && (node->right != NULL)) {
        (*Sum) += node->data;
    }
    ...
    // Gọi đệ quy cho node con trái và node con phải
    duyet(node->left, Sum);
    duyet(node->right, Sum);
}
// Tìm giá trị gần nhất với giá trị nhập vào
TreeNode* searchNear(TreeNode* node, int val) {
    if (node == NULL)
        return NULL;
    ...
    while (node != NULL) {
        if (abs(node->data - val) < abs(closest->data - val)) {
            closest = node;
        }
        ...
        if (val < node->data) {
            ...
            // Nếu giá trị bằng nhau, có thể trả về ngay tại đây nếu muốn
            return node;
        }
    }
