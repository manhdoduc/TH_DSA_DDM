...
int main(){
    ...
    for(int k=0; k<3; k++){
        int comps=0;
        int assigns=0;
        int countC[10000]={0}; // Đã sửa tên mảng thành countC
        int countA[10000]={0}; // Đã sửa tên mảng thành countA
        for(int index=0;index<numK[k];index++){
            int comp=0;
            int assign=0;  // Đã khai báo assign trong vòng lặp
            initializeArray(n, N, -20, 20);
            f(n, N, &comp, &assign);
            countC[index]=comp;
            countA[index]=assign; // Đã sửa tên mảng thành countA
            comps+=comp;
            assigns+=assign;
        }
